...
func (mkSys *MkSys) Build() error {
	//	fmt.Printf("%x", mkSys.Hash())
	//	out := bytes.NewBuffer(mkSys.Output)
	//	log.SetOutput(out)
	...
	if mkSys.Mode != "syscallfiles" && mkSys.OutPath != "" {
		...
		if err != nil {
			return fmt.Errorf("os create %s:%s", mkSys.OutPath, err.Error())
		}
		outfile = of
	}
	...
	if err != nil {
		return fmt.Errorf("os readAll %s:%s", mkSys.SysConf, err.Error())
	}
	...
	case "sys_harvey.s":
		if mkSys.ARCH != "amd64" {
			log.Println("ARCH unsupported or not set")
		}
...
`)
	...
	case "syscallfiles":
		if mkSys.ARCH != "amd64" {
			log.Fatal("ARCH unsupported or not set")
		}
